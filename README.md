...
```bash
mvn clean install               # everything: 318 backend (JUnit) + 97 UI (Vitest), UI bundled into the jar
mvn -pl autopilot-engine test   # run a single module's tests (here, the engine's 144)
```

Backend tests live **with their module** — `autopilot-engine` 144 · `autopilot-adapters` 138 · `autopilot-launcher` 36 (full-boot `@SpringBootTest`); the **97** UI (Vitest) tests run in the launcher's test phase. (`autopilot-core` is ports + value objects, exercised through the modules that use them.)
