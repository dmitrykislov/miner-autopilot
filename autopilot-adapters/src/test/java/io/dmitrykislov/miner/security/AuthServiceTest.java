...
class AuthServiceTest {
    ...
    private final AuthService auth = new AuthService(new AuthProperties(true, HASH, 30, 5, false));
    ...
    @Test void tokenSurvivesAcrossServiceInstances() {
        ...
        String token = new AuthService(new AuthProperties(true, HASH, 30, 5, false)).issueToken();
        assertThat(new AuthService(new AuthProperties(true, HASH, 30, 5, false)).isValidToken(token)).isTrue();
    }
    ...
    @Test void failsClosedWhenNoHashConfigured() {
        ...
        AuthService blank = new AuthService(new AuthProperties(true, "", 30, 5, false));
    ...
    @Test void reportsEnabledFlag() {
        assertThat(new AuthService(new AuthProperties(true, HASH, 30, 5, false)).enabled()).isTrue();
        assertThat(new AuthService(new AuthProperties(false, HASH, 30, 5, false)).enabled()).isFalse();
    }
